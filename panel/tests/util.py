...
hv_available = pytest.mark.skipif(hv is None or hv_version < Version("1.13.0a23"), reason="requires holoviews")
    ...
    while True:
        try:
            ...
            if result not in (None, True, False):
                raise ValueError("`wait_until` callback must return None, True or " f"False, returned {result!r}")
...
def wait_for_port(stdout):
    ...
    if m is None:
        output = "\n    ".join(output)
        pytest.fail("No matching log line in process output, following output " f"was captured:\n\n   {output}")
