...
def test_holoviews_with_widgets(document, comm):
    hmap = hv.HoloMap(
        {(i, chr(65 + i)): hv.Curve([i]) for i in range(3)}, kdims=["X", "Y"]
    )
    ...
    hmap = hv.HoloMap(
        {(i, chr(65 + i)): hv.Curve([i]) for i in range(3)}, kdims=["A", "B"]
    )
    hv_pane.object = hmap
    assert len(hv_pane.widget_box.objects) == 2
...
def test_holoviews_date_slider_widgets_from_holomap():
    hmap = hv.HoloMap(
        {dt.datetime(2016, 1, i + 1): hv.Curve([i]) for i in range(3)}, kdims=["X"]
    )
