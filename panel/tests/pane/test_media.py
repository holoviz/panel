...
def test_local_video_path(document, comm):
    video = Video(ASSETS / 'mp4.mp4')
    model = video.get_root(document, comm=comm)

    assert model.value == 'data:video/mp4;base64,' # noqa

def test_local_video_bytes(document, comm):
    video = Video((ASSETS / 'mp4.mp4').read_bytes())
    model = video.get_root(document, comm=comm)

    assert model.value == 'data:video/mp4;base64,' # noqa

def test_local_video_bytesio(document, comm):
    video = Video(BytesIO((ASSETS / 'mp4.mp4').read_bytes()))
    model = video.get_root(document, comm=comm)
...
def test_local_audio_path(document, comm):
    audio = Audio(ASSETS / 'mp3.mp3')
    model = audio.get_root(document, comm=comm)

    assert model.value == 'data:audio/mp3;base64,/+MYxAAAAANIAAAAAExBTUUzLjk4LjIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA' # noqa

def test_local_audio_bytes(document, comm):
    audio = Audio((ASSETS / 'mp3.mp3').read_bytes())
    model = audio.get_root(document, comm=comm)

    assert model.value == 'data:audio/mp3;base64,/+MYxAAAAANIAAAAAExBTUUzLjk4LjIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA' # noqa

def test_local_audio_bytesio(document, comm):
    audio = Audio(BytesIO((ASSETS / 'mp3.mp3').read_bytes()))
    model = audio.get_root(document, comm=comm)
