...
def test_card_objects(page, card_components):
    ...
    expect(w2_object).to_contain_class('bk-panel-models-widgets-TextInput class_w2')

    w3 = TextInput(label='Text:', css_classes=['class_w3'])
    card.append(w3)

    expect(card_elements).to_have_count(3)
    expect(card_elements.nth(2)).to_contain_class('bk-panel-models-widgets-TextInput class_w3')
...
def test_card_scrollable(page):
    ...
    expect(page.locator('.card')).to_contain_class('bk-panel-models-layout-Card card scrollable-vertical')
