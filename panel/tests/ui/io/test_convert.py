...
if not (PANEL_LOCAL_WHL.is_file() and BOKEH_LOCAL_WHL.is_file()):
    pytest.skip(
        "Skipped because pyodide wheels are not available for current " "version. Build wheels for pyodide using `python scripts/build_pyodide_wheels.py`.",
...
def http_serve():
    ...
    process = Popen(["python", "-m", "http.server", str(HTTP_PORT), "--directory", str(temp_path)], stdout=PIPE)
