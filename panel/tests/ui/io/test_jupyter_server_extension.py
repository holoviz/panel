...
def test_jupyter_server(page, jupyter_preview):
    ...
    assert page.text_content('.string') == '0'

    page.click('.bk-btn')

    wait_until(lambda: page.text_content('.string') == '1', page)

    page.click('.bk-btn')

    wait_until(lambda: page.text_content('.string') == '2', page)
...
def test_jupyter_server_custom_resources(page, jupyter_preview):
    ...
    assert page.locator('.string').evaluate("""(element) =>
...
def test_jupyter_server_kernel_error(page, jupyter_preview):
    ...
    wait_until(lambda: page.text_content('.string') == '0', page)
