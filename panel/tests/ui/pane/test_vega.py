...
def test_altair_select_point(page, dataframe):
    ...
    chart = (
        alt.Chart(dataframe)
        .mark_point(size=12000)
        .encode(x="int", y="float", color=alt.condition(multi, alt.value("black"), alt.value("lightgray")))
        .add_params(multi)
    )
...
def test_altair_select_agg(page, dataframe):
    ...
    chart = (
        alt.Chart(dataframe)
        .mark_bar()
        .transform_aggregate(x="mean(float)", groupby=["str"])
        .encode(x=alt.X("x:Q"), y=alt.Y("str:N"), color=alt.condition(multi_bar_ref, alt.value("black"), alt.value("lightgray")))
