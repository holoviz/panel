...
def test_select_change_options_on_watch(document, comm):
    select = Select(
        options=OrderedDict([("A", "A"), ("1", 1), ("C", object)]),
        value="A",
        name="Select",
    )
...
def test_cross_select_constructor():
    cross_select = CrossSelector(
        options=["A", "B", "C", 1, 2, 3], value=["A", 1], size=5
    )
...
def test_cross_select_move_unselected_to_selected_not_definition_order():
    cross_select = CrossSelector(
        options=["B", "A", "C", 1, 2, 3], value=["A", 1], size=5, definition_order=False
    )
