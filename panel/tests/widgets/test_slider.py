...
def test_date_slider(document, comm):
    ...
    date_slider._comm_change(
        {"value": (datetime(2018, 9, 4) - epoch).total_seconds() * 1000.0}
    )
...
def test_date_range_slider(document, comm):
    ...
    epoch = datetime(1970, 1, 1)
    widget.value = (
        (datetime(2018, 9, 3) - epoch).total_seconds() * 1000,
...
def test_discrete_slider(document, comm):
    discrete_slider = DiscreteSlider(
        name="DiscreteSlider", value=1, options=[0.1, 1, 10, 100]
    )
...
def test_discrete_date_slider(document, comm):
    dates = OrderedDict(
        [("2016-01-0%d" % i, datetime(2016, 1, i)) for i in range(1, 4)]
    )
    discrete_slider = DiscreteSlider(
        name="DiscreteSlider", value=dates["2016-01-02"], options=dates
    )
