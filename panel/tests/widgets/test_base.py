...
all_widgets = [
    w
    for w in param.concrete_descendents(Widget).values()
...
def test_widget_clone_override(widget):
    ...
    assert [
        (k, v)
        for k, v in sorted(w.param.get_param_values())
        if k not in ["name", "width"]
    ] == [
        (k, v)
        for k, v in sorted(clone.param.get_param_values())
        if k not in ["name", "width"]
    ]
