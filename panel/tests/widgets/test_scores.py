...
def test_scores(height=600, sizing_mode="stretch_width", name="Large"):
    ...
    label = pn.indicators.Scores(
        value=labels, color=ACCENT_COLOR, height=height, sizing_mode=sizing_mode, name=name
    )
    return label
...
if __name__.startswith("bokeh"):
    ...
    pn.template.FastListTemplate(
        ...
        sidebar=[
            pn.Param(
                labels, parameters=["top", "color", "value", "sizing_mode", "width", "height"]
            ),
            pn.Param(
                labels_small, parameters=["top", "color", "value", "sizing_mode", "width", "height"]
            ),
        ],
        main=[
            pn.Column(labels, sizing_mode="stretch_both"),
            pn.Column(labels_small, sizing_mode="stretch_both"),
        ],
