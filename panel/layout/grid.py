...
class GridBox(ListPanel):
    ...
    _source_transforms: ClassVar[Mapping[str, str | None]] = {"scroll": None, "objects": None}

    _stylesheets: ClassVar[List[str]] = [f"{CDN_DIST}css/gridbox.css"]
...
class GridSpec(Panel):
    ...
    def __setitem__(self, index, obj):
        ...
        if self._cols_fixed and (l >= self.ncols or r > self.ncols):
            raise IndexError("Assigned object to column(s) out-of-bounds " "of the grid declared by `ncols`. which " f"was set to {self.ncols}.")
        if self._rows_fixed and (t >= self.nrows or b > self.nrows):
            raise IndexError("Assigned object to column(s) out-of-bounds " "of the grid declared by `nrows`, which " f"was set to {self.nrows}.")
        ...
        if overlap_grid.any():
            ...
            overlap_text = (
                ...
                "existing object(s) in the grid:\n\n" + overlapping + "The following shows a view of the grid "
