...
class Convert(Subcommand):
    ...
    args = (
        ...
        ("--to", dict(action="store", type=str, help="The format to convert to, one of 'pyodide' (default), 'pyodide-worker' or 'pyscript'", default="pyodide")),
        (
            "--out",
            dict(
                action="store",
        ...
        ("--disable-http-patch", dict(default=False, action="store_false", help="Whether to disable patching http requests using the pyodide-http library.")),
        ("--watch", dict(action="store_true", help="Watch the files")),
        ("--num-procs", dict(action="store", type=int, default=1, help="The number of processes to start in parallel to convert the apps.")),
    )
    ...
    def invoke(self, args: argparse.Namespace) -> None:
        ...
        if isinstance(requirements, list) and len(requirements) == 1 and pathlib.Path(requirements[0]).is_file():
            req = requirements[0]
            if req.endswith(".txt"):
