...
class Param(PaneBase):
    ...
    def __repr__(self, depth=0):
        ...
        for p, v in sorted(self.get_param_values()):
            if v is self.param[p].default:
                ...
                p == "name" and (v.startswith(obj_cls) or v.startswith(cls))
            ):
                continue
    ...
    def _link_subobjects(self):
        for pname, widget in self._widgets.items():
            ...
            if not any(
                is_parameterized(getattr(w, "value", None))
                or any(is_parameterized(o) for o in getattr(w, "options", []))
                for w in widgets
            ):
                continue
            ...
            def toggle_pane(change, parameter=pname):
                ...
                existing = [
                    p
                    for p in self._expand_layout.objects
    ...
    def widget(self, p_name):
        ...
        if hasattr(p_obj, "get_soft_bounds"):
            ...
            if bounds[1] is not None:
                kw["end"] = bounds[1]
            if ("start" not in kw or "end" not in kw) and not issubclass(
        ...
        if isinstance(widget, Toggle):
            ...
            def link(change, watchers=[watcher]):
                ...
                if change.what == "constant":
                    ...
                    if (
                        change.new < self.display_threshold
                        ...
                        for k in params:
                            if (
                                precedence(k) is None
                                or precedence(k) >= self.display_threshold
                            ):
        ...
        if (is_parameterized(value) or any(is_parameterized(o) for o in options)) and (
            self.expand_button or (self.expand_button is None and not self.expand)
        ):
    ...
    def _ordered_params(self):
        ...
        key_fn = (
            lambda x: x[1].precedence
            if x[1].precedence is not None
            else self.default_precedence
        )
        ...
        ordered_params = [
            el[0] for group in ordered_groups for el in group if el[0] != "name"
        ]
    ...
    def applies(cls, obj):
        return (
            is_parameterized(obj)
            or isinstance(obj, param.parameterized.Parameters)
            or (isinstance(obj, param.Parameter) and obj.owner is not None)
        )
...
class ParamMethod(PaneBase):
    ...
    def __init__(self, object, **params):
        ...
        self._inner_layout = Row(
            self._pane, **{k: v for k, v in params.items() if k in Row.param}
        )
    ...
    def _link_object_params(self):
        ...
        def update_pane(*events):
            ...
            if any(is_parameterized(event.new) for event in events):
                ...
                for p in list(deps):
                    ...
                    for w in list(watchers):
                        if (
                            w.inst is p.inst
                            and w.cls is p.cls
                            and p.name in w.parameter_names
                        ):
                ...
                for _, params in full_groupby(
                    new_deps, lambda x: (x.inst or x.cls, x.what)
                ):
                    p = params[0]
    ...
    @classmethod
    def applies(cls, obj):
        return inspect.ismethod(obj) and isinstance(
