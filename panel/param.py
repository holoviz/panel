...
class Param(PaneBase):
    ...
    default_layout = param.ClassSelector(default=Column, class_=Panel, is_instance=False)

    default_precedence = param.Number(
        default=1e-8,
        precedence=-10,
        doc="""
    ...
    hide_constant = param.Boolean(
        default=False,
        doc="""
        Whether to hide widgets of constant parameters.""",
    )

    initializer = param.Callable(
        default=None,
        doc="""
    ...
    sort = param.ClassSelector(
        default=False,
        class_=(bool, Callable),
        doc="""
    ...
    def __init__(self, object=None, **params):
        ...
        # Construct Layout
        kwargs = {p: v for p, v in self.param.values().items() if p in Layoutable.param and v is not None}
        ...
        if isinstance(layout, Panel):
            ...
            raise ValueError("expand_layout expected to be a panel.layout.Panel" "type or instance, found %s type." % type(layout).__name__)
        self.param.watch(
    ...
    def __repr__(self, depth=0):
        ...
        for p, v in sorted(self.param.values().items()):
            if v == self.param[p].default:
                continue
            elif v is None:
                continue
            elif isinstance(v, str) and v == "":
                continue
            elif p == "object" or (p == "name" and (v.startswith(obj_cls) or v.startswith(cls))):
                continue
    ...
    def _update_widgets(self, *events):
        ...
        widgets = [
            widget
            for p, widget in self._widgets.items()
            if (self.object.param[alias.get(p, p)].precedence is None) or (self.object.param[alias.get(p, p)].precedence >= self.display_threshold)
        ]
    ...
    def widget(self, p_name):
        ...
        if hasattr(p_obj, "get_range"):
            ...
            if "options" in widget_class.param and isinstance(widget_class.param["options"], param.List):
        ...
        if hasattr(p_obj, "get_soft_bounds"):
            ...
            if bounds[1] is not None:
                kw["end"] = bounds[1]
            if ("start" not in kw or "end" not in kw) and not isinstance(p_obj, (param.Date, param.CalendarDate)):
        ...
        def link(change, watchers=[watcher]):
            ...
            if change.what == "constant":
                ...
                if change.new is change.old:
                    ...
                    self._rerender()
                elif change.new < self.display_threshold and widget in self._widget_box.objects:
                ...
                options = p_obj.get_range()
                if "options" in widget.param and isinstance(widget.param["options"], param.List):
        ...
        if (is_parameterized(value) or any(is_parameterized(o) for o in options)) and (self.expand_button or (self.expand_button is None and not self.expand)):
            toggle = Toggle(
                name="\u22EE",
                button_type="primary",
            ...
            width = widget.width
            widget.param.update(margin=(5, 0, 5, 10), sizing_mode="stretch_width", width=None)
    ...
    def _ordered_params(self):
        params = [(p, pobj) for p, pobj in self.object.param.objects("existing").items() if p in self.parameters or p == "name"]
        if self.sort:
        ...
        ordered_params = [el[0] for group in ordered_groups for el in group if (el[0] != "name" or el[0] in self.parameters)]
...
class ParamMethod(ReplacementPane):
    ...
    loading_indicator = param.Boolean(
        default=config.loading_indicator,
        doc="""
...
class ParamFunction(ParamMethod):
    ...
    def _link_object_params(self):
        ...
        for group in grouped.values():
            ...
            if isinstance(pobj, Reactive) and self.loading_indicator:
                props = {dep.name: "loading" for dep in group if dep.name in pobj._linkable_params}
                if props:
                    pobj.jslink(self._inner_layout, **props)
            self._internal_callbacks.append(watcher)
...
class ReactiveExpr(PaneBase):
    ...
    widget_location = param.Selector(
        default="left",
        objects=["left", "right", "top", "bottom", "top_left", "top_right", "bottom_left", "bottom_right", "left_top", "right_top", "right_bottom"],
        doc="""
...
__all__ = ("Param", "ParamFunction", "ParamMethod", "ReactiveExpr", "set_values")
