...
def assert_source_syncable(source: "Reactive", properties: Iterable[str]) -> None:
    for prop in properties:
        if prop.startswith("event:"):
            ...
            raise ValueError(
                f"Cannot jslink {prop!r} parameter on {type(source).__name__} " "object, the parameter requires a live Python kernel " "to have an effect."
            )
...
class Callback(param.Parameterized):
    ...
    def __init__(self, source: "Reactive", target: "JSLinkTarget" = None, args: Dict[str, Any] = None, code: Dict[str, str] = None, **params):
        """
        ...
        if source in self.registry:
            ...
            for link in links:
                ...
                if not hasattr(link, "target"):
                    pass
                elif type(link) is type(self) and link.source is source and link.target is self.target and params == link_params:
                    return
    ...
    @classmethod
    def _process_callbacks(cls, root_view: Viewable, root_model: BkModel, changed: Viewable | None = None, old_models=None):
        if not root_model:
            return
        ...
        found = [
            ...
            if not link._requires_target or link.target in linkable or isinstance(link.target, param.Parameterized)
        ]
        ...
        if "holoviews" in sys.modules:
            ...
            found = [(link, src, tgt) for (link, src, tgt) in found if not (isinstance(src, Dimensioned) or isinstance(tgt, Dimensioned))]
        ...
        for link, src, tgt in found:
            ...
            if (
                (src is None or ref not in getattr(src, "_models", [ref]))
                or (getattr(link, "_requires_target", False) and tgt is None)
                or (tgt is not None and ref not in getattr(tgt, "_models", [ref]))
            ):
                continue
...
class Link(Callback):
    ...
    def link(self) -> None:
        ...
        if source in self.registry:
            ...
            for link in links:
                link_params = {k: v for k, v in link.param.values().items() if k != "name"}
                if type(link) is type(self) and link.source is source and link.target is self.target and params == link_params:
                    return
            self.registry[source].append(self)
        else:
            self.registry[source] = [self]
...
class CallbackGenerator(object):
    ...
    def _init_callback(
        ...
        link: "Link",
        source: "Reactive",
        src_spec: "SourceModelSpec",
        target: "JSLinkTarget" | None,
        tgt_spec: "TargetModelSpec",
        code: Optional[str],
    ) -> None:
    ...
    def _initialize_models(
        self, link, source: "Reactive", src_model: "Model", src_spec: str, target: "JSLinkTarget" | None, tgt_model: "Model" | None, tgt_spec: str | None
    ) -> None:
        """
    ...
    def _get_code(self, link: "Link", source: "JSLinkTarget", src_spec: str, target: "JSLinkTarget" | None, tgt_spec: str | None) -> str:
        ...
        if tgt_spec == "loading":
            ...
            return self._loading_link_template.format(
                src_attr=src_spec, src_transform=src_transform, loading_spinner=config.loading_spinner, loading_css_class=LOADING_INDICATOR_CSS_CLASS
            )
        else:
...
__all__ = ("Callback", "CallbackGenerator", "JSCallbackGenerator", "JSLinkCallbackGenerator", "Link")
