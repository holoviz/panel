...
class LaTeX(DivPaneBase):
    ...
    def _get_model_type(self, comm):
        ...
        if module is None:
            if (
                "panel.models.mathjax" in sys.modules
                and "panel.models.katex" not in sys.modules
            ):
                module = "mathjax"
            else:
                module = "katex"
        ...
        if "panel.models." + module not in sys.modules:
            if isinstance(comm, JupyterComm):
                self.param.warning(
                    "{model} model was not imported on instantiation "
                    "and may not render in a notebook. Restart "
                    "the notebook kernel and ensure you load "
                    "it as part of the extension using:"
                    "\n\npn.extension('{module}')\n".format(module=module, model=model)
                )
