...
class HTML(HTMLBasePane):
    ...
    _rename: ClassVar[Mapping[str, str | None]] = {"sanitize_html": None, "sanitize_hook": None}
...
class DataFrame(HTML):
    ...
    def applies(cls, obj: Any) -> float | bool | None:
        ...
        if any(m in module for m in ("pandas", "dask", "streamz")) and name in ("DataFrame", "Series", "Random", "DataFrames", "Seriess", "Styler"):
            return 0.3
        else:
            return False
    ...
    def _transform_object(self, obj: Any) -> Dict[str, Any]:
        ...
        if hasattr(obj, "to_html"):
            if "dask" in module:
                ...
                kwargs = {p: getattr(self, p) for p in self._rerender_params if p not in HTMLBasePane.param and p != "_object"}
...
class Markdown(HTMLBasePane):
    ...
    def _get_parser(cls, renderer, plugins, **renderer_options):
        ...
        parser = parser.enable("strikethrough").enable("table").use(anchors_plugin, permalink=True).use(deflist_plugin).use(footnote_plugin).use(tasklists_plugin)
    ...
    def _transform_object(self, obj: Any) -> Dict[str, Any]:
        ...
        if self.renderer == "markdown":
            html = markdown.markdown(obj, extensions=self.extensions, output_format="html5", **self.renderer_options)
        else:
            html = self._get_parser(self.renderer, tuple(self.plugins), **self.renderer_options).render(obj)
        return dict(object=escape(html))
