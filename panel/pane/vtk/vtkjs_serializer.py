...
def _dump_poly_data(scDirs, datasetDir, dataDir, dataset, colorArrayInfo, root):
    ...
    # Points
    root["points"] = _dump_data_array(
        scDirs, datasetDir, dataDir, dataset.GetPoints().GetData()
    )
    ...
    for cell_type in ["verts", "lines", "polys", "strips"]:
        ...
        if cell and cell.GetData().GetNumberOfTuples() > 0:
            root[cell_type] = _dump_data_array(
                scDirs, datasetDir, dataDir, cell.GetData()
            )
...
def vtk_lut_to_palette(lut):
    ...
    palette = [
        "#{0:02x}{1:02x}{2:02x}".format(int(255 * r), int(255 * g), int(255 * b))
                    ...
                    if scalar_visibility:
                        ...
                        if dataArray:
                            ...
                            if array_name and legend is not None:
                                legend.update(
                                    {array_name: vtk_lut_to_palette(lookupTable)}
                                )
    return legend
                ...
                if dataset and dataset.GetPoints():
                    ...
                    if scalar_visibility:
                        ...
                        if dataArray:
                            if dataArray.GetLookupTable():
                                lookupTable = dataArray.GetLookupTable()
                                colorArray = dataArray.GetLookupTable().MapScalars(
                    ...
                    _write_data_set(
                        scDirs, dataset, colorArrayInfo, newDSName=componentName
                    )
                    ...
                    sceneComponents.append(
                        {
                            ...
                            "actor": {
                                # vtkProp
                                "visibility": renProp.GetVisibility()
                                if renProp.IsA("vtkProp")
                                else 0,
                                "pickable": renProp.GetPickable()
                                if renProp.IsA("vtkProp")
                                else 0,
                                "dragable": renProp.GetDragable()
                                if renProp.IsA("vtkProp")
                                else 0,
                                "useBounds": renProp.GetUseBounds()
                                if renProp.IsA("vtkProp")
                                else 0,
                                # vtkProp3D
                                "origin": renProp.GetOrigin()
                                if renProp.IsA("vtkProp3D")
                                else [0, 0, 0],
                                "scale": renProp.GetScale()
                                if renProp.IsA("vtkProp3D")
                                else [1, 1, 1],
                                "position": renProp.GetPosition()
                                if renProp.IsA("vtkProp3D")
                                else [0, 0, 0],
                                # vtkActor
                                "forceOpaque": renProp.GetForceOpaque()
                                if renProp.IsA("vtkActor")
                                else 0,
                                "forceTranslucent": renProp.GetForceTranslucent()
                                if renProp.IsA("vtkActor")
                                else 0,
                            },
                            "actorRotation": renProp.GetOrientationWXYZ()
                            if renProp.IsA("vtkProp3D")
                            else [0, 0, 0, 0],
                            "mapper": {
