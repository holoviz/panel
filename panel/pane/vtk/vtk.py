...
class VTKVolume(PaneBase):
    ...
    def applies(cls, obj):
        ...
        if (isinstance(obj, np.ndarray) and obj.ndim == 3) or any(
            [isinstance(obj, k) for k in cls._serializers.keys()]
        ):
            return True
    ...
    def _init_properties(self):
        return {
            ...
            if v is not None
            and k
            not in ["default_layout", "object", "max_data_size", "spacing", "origin"]
        }
    ...
    def _volume_from_array(self, sub_array):
        return dict(
            buffer=base64encode(
                sub_array.ravel(order="F" if sub_array.flags["F_CONTIGUOUS"] else "C")
            ),
            dims=sub_array.shape
            if sub_array.flags["F_CONTIGUOUS"]
            else sub_array.shape[::-1],
            spacing=self._sub_spacing
            if sub_array.flags["F_CONTIGUOUS"]
            else self._sub_spacing[::-1],
            origin=self.origin,
    ...
    def _get_volume_data(self):
        if self.object is None:
            ...
            available_serializer = [
                v
                for k, v in VTKVolume._serializers.items()
        ...
        if any([d_f > 1 for d_f in dowsnscale_factor]):
            try:
                ...
                sub_array = nd.interpolation.zoom(
                    array, zoom=[1 / d_f for d_f in dowsnscale_factor], order=0
                )
            except ImportError:
                sub_array = array[
                    :: int(np.ceil(dowsnscale_factor[0])),
                    :: int(np.ceil(dowsnscale_factor[1])),
                    :: int(np.ceil(dowsnscale_factor[2])),
                ]
            self._sub_spacing = tuple(
                e / (s - 1) for e, s in zip(extent, sub_array.shape)
            )
        else:
            sub_array = array
            self._sub_spacing = self.spacing
                    ...
                    cbs.append(
                        ColorBar(
                            color_mapper=LinearColorMapper(
                                low=v["low"], high=v["high"], palette=v["palette"]
                            ),
                            title=k,
    ...
    def _get_vtkjs(self):
        if self._vtkjs is None and self.object is not None:
            if isinstance(self.object, string_types) and self.object.endswith(".vtkjs"):
                ...
                available_serializer = [
                    v for k, v in VTK._serializers.items() if isinstance(self.object, k)
                ]
                if len(available_serializer) == 0:
                    import vtk
                    from .vtkjs_serializer import render_window_serializer

                    VTK.register_serializer(
                        vtk.vtkRenderWindow, render_window_serializer
                    )
