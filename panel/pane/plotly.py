...
class Plotly(PaneBase):
    ...
    viewport_update_policy = param.Selector(objects=["mouseup", "continuous", "throttle"],
                                            default="mouseup", doc="""
        Policy by which the viewport parameter is updated during user interactions:
         ...
         - "throttle": updates are synchronized while panning, at intervals determined by
                       the viewport_update_throttle parameter""")

    viewport_update_throttle = param.Integer(bounds=(0, None), default=200, doc="""
