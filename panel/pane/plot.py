    ...
    def _sync_properties(self):
        ...
        try:
            self.param.update(
                {
                    p: v
                    for p, v in self.object.properties_with_values().items()
                    if p not in self._overrides and p in Layoutable.param and p not in ("css_classes", "name")
                }
            )
...
class Matplotlib(Image, IPyWidget):
    ...
    encode = param.Boolean(
        default=True,
        doc="""
        Whether to encode SVG out as base64.""",
    )

    format = param.Selector(
        default="png",
        objects=["png", "svg"],
        doc="""
    ...
    _rerender_params = PNG._rerender_params + ["interactive", "object", "dpi", "tight", "high_dpi"]
    ...
    def applies(cls, obj: Any) -> float | bool | None:
        ...
        if is_fig and obj.canvas is None:
            raise ValueError("Matplotlib figure has no canvas and " "cannot be rendered.")
        return is_fig
    ...
    def _get_model(self, doc: Document, root: Optional[Model] = None, parent: Optional[Model] = None, comm: Optional[Comm] = None) -> Model:
        ...
        del properties["text"]
        model = self._get_ipywidget(manager.canvas, doc, root, comm, **properties)
        root = root or model
    ...
    def _update(self, ref: str, model: Model) -> None:
        ...
        if self.object is not manager.canvas.figure:
            ...
            event = {"width": manager.canvas._width, "height": manager.canvas._height}
    ...
    def _data(self, obj):
        ...
        obj.canvas.print_figure(b, format=self.format, facecolor=obj.get_facecolor(), edgecolor=obj.get_edgecolor(), dpi=self.dpi, bbox_inches=bbox_inches)
        return b.getvalue()
...
class Folium(HTML):
    ...
    sizing_mode = param.ObjectSelector(
        default="stretch_width", objects=["fixed", "stretch_width", "stretch_height", "stretch_both", "scale_width", "scale_height", "scale_both", None]
    )
