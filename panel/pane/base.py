...
class PaneBase(Reactive):
    ...
    def __init__(self, object=None, **params):
        ...
        self._internal_callbacks.extend(
            [self.param.watch(self._sync_layoutable, list(Layoutable.param)), self.param.watch(self._update_pane, self._rerender_params)]
        )
    ...
    def _update_object(self, ref: str, doc: "Document", root: Model, parent: Model, comm: Comm | None) -> None:
        ...
        try:
            ...
            if parent is layout_parent:
                parent.update(
                    **self.layout._compute_sizing_mode(
                        parent.children,
                        dict(
...
class ReplacementPane(PaneBase):
    ...
    def _update_from_object(cls, object: Any, old_object: Any, was_internal: bool, inplace: bool = False, **kwargs):
        ...
        if type(old_object) is pane_type and ((not links and not custom_watchers and was_internal) or inplace):
            ...
            pane = panel(object, **{k: v for k, v in kwargs.items() if k in pane_type.param})
            if pane is object:
