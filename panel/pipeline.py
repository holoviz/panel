...
class Pipeline(param.Parameterized):
    ...
    def __init__(self, stages=[], **params):
        ...
        self._spinner_layout = Row(
            HSpacer(), Column(VSpacer(), spinner, VSpacer()), HSpacer()
        )
        stage_layout = Row()
        if len(stages):
    ...
    def _validate(self, stage):
        if any(stage is s for n, s in self._stages):
            ...
            (isinstance(stage, type) and issubclass(stage, param.Parameterized))
            or isinstance(stage, param.Parameterized)
        ):
            raise ValueError(
                "Pipeline stages must be Parameterized classes or instances."
            )
    ...
    def _init_stage(self):
        ...
        if self._state:
            ...
            if self.inherit_params:
                params = [
                    k
                    for k, v in self._state.param.objects("existing").items()
                    if v.precedence is None or v.precedence >= 0
                ]
                kwargs.update(
                    {
                        k: v
                        for k, v in self._state.param.get_param_values()
    ...
    def _previous(self):
        ...
        try:
            ...
            self._error.object = (
                "Previous stage raised following error:\n\n\t%s: %s"
    ...
    def _make_progress(self):
        ...
        point_labels = points.add_dimension(
            "text", 0, [n for n, _ in self._stages], vdim=True
        )
