...
def get_root(graph):
    ...
    if len(roots) > 1:
        raise ValueError("Graph has more than one node with no " "incoming edges. Ensure that the graph " "only has a single source node.")
    elif len(roots) == 0:
        raise ValueError("Graph has no source node. Ensure that the " "graph is not cyclic and has a single " "starting point.")
    return roots[0]
...
class Pipeline(Viewer):
    ...
    * stage: The contents of the current pipeline stage.
    ...
    def __init__(self, stages=[], graph={}, **params):
        ...
        self.header = Row(Column(self.title, self.error), self.network, self.buttons, sizing_mode="stretch_width")
    ...
    def _validate(self, stage):
        ...
        elif not ((isinstance(stage, type) and issubclass(stage, param.Parameterized)) or isinstance(stage, param.Parameterized)):
    ...
    def _next(self):
        ...
        try:
            ...
            stage_kwargs = self._stages[self._stage][-1]
            ready_param = stage_kwargs.get("ready_parameter", self.ready_parameter)
            if ready_param and getattr(self._state, ready_param, False) and stage_kwargs.get("auto_advance", self.auto_advance):
                self._next()
        finally:
            self._update_progress()
    ...
    def _make_progress(self):
        ...
        nodes = hv.Nodes(nodes, ["x", "y", "Stage"], "State").opts(
            alpha=0, default_tools=["tap"], hooks=[tap_renderer], hover_alpha=0, selection_alpha=0, nonselection_alpha=0, axiswise=True, size=10, backend="bokeh"
        )
        ...
        labels = hv.Labels(nodes, ["x", "y"], "Stage").opts(yoffset=-0.30, default_tools=[], axiswise=True, backend="bokeh")
    ...
    def define_graph(self, graph, force=False):
        ...
        if not_found:
            raise ValueError(
                ...
                "referenced in the graph have been added." % (not_found[0] if len(not_found) == 1 else not_found)
            )
