...
def server_html_page_for_session(
    ...
    with set_curdoc(doc):
        ...
        if config.global_loading_spinner:
            html = html.replace("<body>", f'<body class="{LOADING_INDICATOR_CSS_CLASS} pn-{config.loading_spinner}">')
    return html
    ...
    async def get(self, *args, **kwargs):
        ...
        with self._session_prefix():
            ...
            if old_request and state._sessions.get(key_func(self.request)) is session:
                session_id = generate_session_id(secret_key=self.application.secret_key, signed=self.application.sign_sessions)
                payload = get_token_payload(session.token)
                payload.update(self._token_payload())
                del payload["session_expiry"]
                token = generate_jwt_token(
                    session_id, secret_key=app.secret_key, signed=app.sign_sessions, expiration=app.session_token_expiration, extra_payload=payload
                )
            else:
                token = session.token
                ...
                if auth_cb:
                    ...
                    try:
                        ...
                        if isinstance(authorized, str):
                            ...
                            auth_error = f'Authorization callback errored. Could not validate user name "{state.user}" ' f'for the given app "{self.request.path}".'
                        if authorized:
                            auth_error = None
                    except Exception:
...
class AutoloadJsHandler(BkAutoloadJsHandler, SessionPrefixHandler):
    ...
    async def get(self, *args, **kwargs) -> None:
        ...
        with self._session_prefix():
            session = await self.get_session()
            with set_curdoc(session.document):
                resources = Resources.from_bokeh(self.application.resources(server_url), absolute=True)
                js = autoload_js_script(session.document, resources, session.token, element_id, app_path, absolute_url, absolute=True)
...
def modify_document(self, doc: "Document"):
    ...
    try:
        ...
        def handle_exception(handler, e):
            ...
            Alert(
                f'<b>{type(e).__name__}</b>: {e_msg}\n<pre style="overflow-y: auto">{tb}</pre>', alert_type="danger", margin=5, sizing_mode="stretch_width"
            ).servable()
...
if (
    ...
    and type(asyncio.get_event_loop_policy()) is asyncio.WindowsSelectorEventLoopPolicy
    and (("jupyter_server" not in sys.modules and "jupyter_client" not in sys.modules) or "pytest" in sys.modules)
):
...
def serve(
    ...
    **kwargs,
    ...
    if threaded:
        ...
        server = StoppableThread(target=get_server, io_loop=loop, args=(panels,), kwargs=kwargs)
...
def get_static_routes(static_dirs):
    ...
    for slug, path in static_dirs.items():
        ...
        patterns.append((r"%s/(.*)" % slug, StaticFileHandler, {"path": path}))
...
def get_server(
    ...
    **kwargs,
    ...
    if isinstance(panel, dict):
        ...
        for slug, app in panel.items():
            ...
            if isinstance(title, dict):
                try:
                    ...
                    raise KeyError("Keys of the title dictionary and of the apps " f"dictionary must match. No {slug} key found in the " "title dictionary.")
            else:
                title_ = title
            ...
            if "flask" in sys.modules:
                ...
                if isinstance(app, Flask):
                    ...
                    if not slug.endswith("/"):
                        slug += "/"
                    extra_patterns.append(("^" + slug + ".*", ProxyFallbackHandler, dict(fallback=wsgi, proxy=slug)))
                    continue
            if isinstance(app, pathlib.Path):
                app = str(app)  # enables serving apps from Paths
            if isinstance(app, str) and (app.endswith(".py") or app.endswith(".ipynb") or app.endswith(".md")) and os.path.isfile(app):
        ...
        if isinstance(panel, str) and (panel.endswith(".py") or panel.endswith(".ipynb") or panel.endswith(".md")) and os.path.isfile(panel):
    ...
    if admin:
        if "/admin" in apps:
            raise ValueError("Cannot enable admin panel because another app is being served " "on the /admin endpoint")
    ...
    if start:
        ...
        try:
            ...
            warn("IOLoop couldn't be started. Ensure it is started by " "process invoking the panel.io.server.serve.")
    return server
