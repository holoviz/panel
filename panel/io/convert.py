...
def make_index(files, title=None, manifest=True):
    ...
    return INDEX_TEMPLATE.render(items=items, manifest=manifest, apple_icon=apple_icon, favicon=favicon, title=title, PANEL_CDN=CDN_DIST)
...
def script_to_html(
    ...
    if requirements == "auto":
        ...
        try:
            ...
            requirements = [r2 for r in requirements if (r2 := r.split("#")[0].strip()) and Requirement(r2)]
        except Exception as e:
            raise ValueError(f"Requirements parser raised following error: {e}")
    ...
    reqs = base_reqs + [req for req in requirements if req not in ("panel", "bokeh")]
    ...
    if runtime == "pyscript":
        ...
        if runtime == "pyodide-worker":
            if js_resources == "auto":
                js_resources = []
            worker_handler = WORKER_HANDLER_TEMPLATE.render({"name": app_name, "loading_spinner": config.loading_spinner})
            web_worker = WEB_WORKER_TEMPLATE.render({"PYODIDE_URL": PYODIDE_PYC_URL if compiled else PYODIDE_URL, "env_spec": env_spec, "code": code})
            ...
            plot_script = script_template.render({"env_spec": env_spec, "code": code})
    ...
    if prerender:
        ...
        render_item = RenderItem(token="", roots=document.roots, use_for_title=False)
    ...
    spinner_css = loading_css(config.loading_spinner, config.loading_color, config.loading_max_height)
    css_resources.append(f'<style type="text/css">\n{loading_base}\n{spinner_css}\n</style>')
    with set_curdoc(document):
...
def _convert_process_pool(
    apps: List[str], dest_path: str | None = None, max_workers: int = 4, requirements: List[str] | Literal["auto"] | os.PathLike = "auto", **kwargs
):
    ...
    for group in groups:
        with ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
            ...
            for app in group:
                ...
                f = executor.submit(convert_app, app, dest_path, requirements=app_requires, **kwargs)
                futures.append(f)
