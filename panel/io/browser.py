...
class BrowserInfo(Syncable):
    ...
    dark_mode = param.Boolean(
        default=None,
        doc="""
        Whether the user prefers dark mode.""",
    )

    device_pixel_ratio = param.Number(
        default=None,
        doc="""
    ...
    timezone_offset = param.Number(
        default=None,
        doc="""
        The time offset from UTC in minutes.""",
    )

    webdriver = param.Boolean(
        default=None,
        doc="""
    ...
    def get_root(self, doc: Document | None = None, comm: Comm | None = None, preprocess: bool = True) -> "Model":
