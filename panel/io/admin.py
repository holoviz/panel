...
download_filename, download_button = log_terminal.download_menu(
    text_kwargs={"name": "Enter filename for logfile", "value": "log.csv"}, button_kwargs={"name": "Download logfile"}
)


EVENT_TYPES = {"initializing": "MediumSeaGreen", "destroyed": "red", "rendering": "Orange", "processing": "DodgerBlue", "periodic": "Violet", "logging": "white"}
...
def get_timeline(doc=None):
    ...
    cds = ColumnDataSource(data={"x0": [], "x1": [], "y0": [], "y1": [], "msg": [], "session": [], "color": [], "line_color": [], "type": []})
    p.yaxis.axis_label = "Sessions"
    p.quad(left="x0", right="x1", top="y1", bottom="y0", source=cds, line_color="line_color", fill_color="color", alpha=0.7, legend_field="type")
...
def get_process_info():
    memory = Trend(data=get_mem(), plot_x="time", plot_y="memory", plot_type="step", name="Memory Usage (MB)", width=300, height=300)
    cpu = Trend(data=get_cpu(), plot_x="time", plot_y="cpu", plot_type="step", name="CPU Usage (%)", width=300, height=300)
...
def admin_template(doc):
    ...
    if config.profiler:
        tabs.append(("Launch Profiling", profiling_tabs(state, r"^\/.*", None)))
    tabs.extend([("User Profiling", profiling_tabs(state, None, r"^\/.*")), ("Logs", log_component())])
