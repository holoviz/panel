...
_NATIVE_TYPES = (bytes, str, float, int, bool, bytearray, type(None))
...
def _generate_hash_inner(obj):
    ...
    if hash_func is not None:
        try:
            ...
            raise ValueError(f"User hash function {hash_func!r} failed for input " f'{obj!r} with following error: {type(e).__name__}("{e}").')
        return output
...
def _key(obj):
    if obj is None:
        ...
        _get_fqn(obj) == "pandas.core.frame.DataFrame" or _get_fqn(obj) == "numpy.ndarray" or inspect.isbuiltin(obj) or inspect.isroutine(obj) or inspect.iscode(obj)
    ):
        return id(obj)
...
def cache(func=None, hash_funcs=None, max_items=None, policy="LRU", ttl=None, to_disk=False, cache_path="./cache", per_session=False):
    """
    ...
    if policy.lower() not in ("fifo", "lru", "lfu"):
        raise ValueError(f"Cache policy must be one of 'FIFO', 'LRU' or 'LFU', not {policy}.")

    hash_funcs = hash_funcs or {}
    if func is None:
        return lambda f: cache(func=f, hash_funcs=hash_funcs, max_items=max_items, ttl=ttl, to_disk=to_disk, cache_path=cache_path)
    func_hash = None  # noqa
    ...
    def hash_func(*args, **kwargs):
        ...
        is_method = args and isinstance(args[0], object) and getattr(type(args[0]), func_name, None) is wrapped_func
