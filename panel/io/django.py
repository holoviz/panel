...
async def doc_handle(self, body):
    ...
    page = server_html_page_for_session(
        session, resources=resources, title=session.document.title, template=session.document.template, template_variables=session.document.template_variables
    )
...
async def autoload_handle(self, body):
    with _session_prefix(self):
        ...
        js = autoload_js_script(session.document, resources, session.token, element_id, app_path, absolute_url, absolute=absolute)
