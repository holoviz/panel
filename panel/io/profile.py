...
def render_snakeviz(name, sessions):
    ...
    for session in sessions[1:]:
        pstats.add(session)
    rendered = (
        SNAKEVIZ_TEMPLATE.generate(profile_name=name, table_rows=table_rows(pstats), callees=json_stats(pstats))
        .decode("utf-8")
        .replace("/static/", "/snakeviz/static/")
    )
...
def get_profiles(profilers, **kwargs):
    ...
    for (path, engine), sessions in profilers.items():
        ...
        html = HTML(
            f'<iframe srcdoc="{src}" width="100%" height="100%" frameBorder="0" style="{style}"></iframe>', sizing_mode="stretch_both", margin=0, min_height=800
        )
...
def get_sessions(allow=None, deny=None):
    return {(name, e): ps for (name, e), ps in state._profiles.items() if (not allow or re.match(allow, name)) and (not deny or not re.match(deny, name))}
...
def profiling_tabs(state, allow=None, deny=[]):
    ...
    return Column(
        Accordion(("Config", config_panel), active=[], active_header_background="#444444", header_background="#333333", sizing_mode="stretch_width", margin=0),
        tabs,
