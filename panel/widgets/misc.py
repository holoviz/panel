...
class FileDownload(IconMixin):
    ...
    button_style = param.ObjectSelector(
        default="solid",
        objects=BUTTON_STYLES,
        doc="""
    ...
    _mime_types = {
        ...
        "text": {"css": "css", "csv": "plain;charset=UTF-8", "js": "javascript", "html": "html", "txt": "plain;charset=UTF-8"},
        "video": {"mp4": "mp4", "ogg": "ogg", "webm": "webm"},
    }

    _rename: ClassVar[Mapping[str, str | None]] = {"callback": None, "button_style": None, "file": None, "_clicks": "clicks"}
    ...
    def _transfer(self):
        if self.file is None and self.callback is None:
            if self.embed:
                raise ValueError("Must provide a file or a callback " "if it is to be embedded.")
            return
...
class JSONEditor(Widget):
    ...
    mode = param.Selector(
        default="tree",
        objects=["tree", "view", "form", "text", "preview"],
        doc="""
