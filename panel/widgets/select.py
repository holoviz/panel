...
from bokeh.models.widgets import (
    ...
    MultiSelect as _BkMultiSelect,
    RadioButtonGroup as _BkRadioButtonGroup,
...
class _CheckGroupBase(Select):
    ...
    def _process_param_change(self, msg):
        ...
        if "value" in msg:
            msg["active"] = [
                indexOf(v, values) for v in msg.pop("value") if isIn(v, values)
            ]
        if "options" in msg:
            msg["labels"] = list(msg.pop("options"))
            if any(not isIn(v, values) for v in self.value):
...
class ToggleGroup(Select):
    ...
    def __new__(cls, widget_type="button", behavior="check", **params):
        ...
        if widget_type not in ToggleGroup._widgets_type:
            raise ValueError(
                "widget_type {} is not valid. Valid options are {}".format(
                    widget_type, ToggleGroup._widgets_type
                )
            )
        if behavior not in ToggleGroup._behaviors:
            raise ValueError(
                "behavior {} is not valid. Valid options are {}".format(
                    widget_type, ToggleGroup._behaviors
                )
            )
        ...
        if behavior == "check":
            ...
            if isinstance(params.get("value"), list):
                raise ValueError(
                    "Radio buttons require a single value, "
...
class CrossSelector(CompositeWidget, MultiSelect):
    ...
    def __init__(self, *args, **kwargs):
        ...
        labels, values = self.labels, self.values
        selected = [
            labels[indexOf(v, values)]
            for v in kwargs.get("value", [])
            if isIn(v, values)
        ]
        ...
        self._composite = Row(
            blacklist,
            Column(VSpacer(), buttons, VSpacer()),
            whitelist,
