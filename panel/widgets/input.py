...
class FileInput(Widget):
    ...
    filename = param.ClassSelector(default=None, class_=(str, list), is_instance=True)

    mime_type = param.ClassSelector(default=None, class_=(str, list), is_instance=True)
    ...
    _source_transforms: ClassVar[Mapping[str, str | None]] = {"value": "'data:' + source.mime_type + ';base64,' + value"}
    ...
    def save(self, filename):
        ...
        if isinstance(filename, list) and not isinstance(value, list):
            raise TypeError(
                "FileInput contains a list of files but only a single " "filename was given. Please provide a list of filenames or " "file-like objects."
            )
        elif not isinstance(filename, list) and isinstance(value, list):
            raise TypeError("FileInput contains a single files but a list of " "filenames was given. Please provide a single filename " "or file-like object.")
...
class DatePicker(Widget):
    ...
    _rename: ClassVar[Mapping[str, str | None]] = {"start": "min_date", "end": "max_date"}
...
class _DatetimePickerBase(Widget):
    ...
    enable_time = param.Boolean(
        default=True,
        doc="""
      Enable editing of the time in the widget.""",
    )

    enable_seconds = param.Boolean(
        default=True,
        doc="""
    ...
    _source_transforms: ClassVar[Mapping[str, str | None]] = {"value": None, "start": None, "end": None, "mode": None}
...
class DatetimeRangePicker(_DatetimePickerBase):
    ...
    def _serialize_value(self, value):
        if isinstance(value, str) and value:
            value = [datetime.strptime(value, r"%Y-%m-%d %H:%M:%S") for value in value.split(" to ")]
...
class _SpinnerBase(_NumericInputBase):
    page_step_multiplier = param.Integer(
        default=10,
        bounds=(0, None),
        doc="""
    ...
    def _process_param_change(self, msg):
        ...
        if value.size <= self.max_array_size:
            ...
            if not self.disabled:
                self.param.warning(f"Number of array elements ({value.size}) exceeds " f"`max_array_size` ({self.max_array_size}), editing " "will be disabled.")
                self.disabled = True
                msg["disabled"] = True
...
class Switch(_BooleanWidget):
    ...
    _rename: ClassVar[Mapping[str, str | None]] = {"name": None, "value": "active"}
