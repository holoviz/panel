...
class _config(param.Parameterized):
    ...
    def set(self, **kwargs):
        ...
        overrides = [
            (k, getattr(self, k + "_")) for k in self.param if k.startswith("_")
        ]
    ...
    def embed_json(self):
        if self._embed_json_ is not None:
            ...
            return (
                os.environ.get("PANEL_EMBED_JSON", _config._embed_json) in self._truthy
            )
...
config = _config(
    **{
        k: None if p.allow_None else getattr(_config, k)
    ...
    def __call__(self, *args, **params):
        ...
        for arg in args:
            if arg not in self._imports:
                self.param.warning(
                    "%s extension not recognized and " "will be skipped." % arg
                )
            else:
        ...
        for k, v in params.items():
            if k in ["raw_css", "css_files"]:
                if not isinstance(v, list):
                    raise ValueError(
                        "%s should be supplied as a list, "
                        "not as a %s type." % (k, type(v).__name__)
                    )
        ...
        if hasattr(ip, "kernel") and not self._loaded:
            ...
            _JupyterCommManager.get_client_comm(
                self._process_comm_msg, "hv-extension-comm"
            )
