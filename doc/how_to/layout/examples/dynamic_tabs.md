...
```
## Using callables with `Tabs`

Tabs can also render content generated by functions. When combined with `ParamFunction` or `ParamMethod`, the content is only computed when the tab is selected. This allows expensive computations to be deferred until they are actually needed.
...
class Example(param.Parameterized):
    ...
    @param.depends()
    def tab1(self):
        return "Content generated when Tab 1 is opened"

    @param.depends()
    def tab2(self):
        return "Content generated when Tab 2 is opened"

example = Example()

tabs = pn.Tabs(
    ("Tab 1", pn.param.ParamFunction(example.tab1)),
    ("Tab 2", pn.param.ParamFunction(example.tab2)),
    dynamic=True
)
...
Using callables with `Tabs` can be useful when the content of a tab is expensive to compute, since the computation will only happen when the user selects the tab.
