...
def get_widgets(model, skip_none=True, **kwargs):
    ...
    for p, v in model.properties_with_values().items():
        ...
        if 'alpha' in p:
            w = pn.ui.FloatSlider(start=0, end=1, **ps)
        elif 'color' in p:
            if v in list(NamedColor):
                w = pn.ui.Select(options=list(NamedColor), **ps)
            else:
                w = pn.ui.ColorPicker(**ps)
        elif p=="width":
            w = pn.ui.IntSlider(start=400, end=800, **ps)
        elif p in ["inner_width", "outer_width"]:
            w = pn.ui.IntSlider(start=0, end=20, **ps)
        elif p.endswith('width'):
            w = pn.ui.FloatSlider(start=0, end=20, **ps)
        elif 'marker' in p:
            w = pn.ui.Select(options=list(MarkerType), **ps)
        elif p.endswith('cap'):
            w = pn.ui.Select(options=list(LineCap), **ps)
        elif p == 'size':
            w = pn.ui.FloatSlider(start=0, end=20, **ps)
        elif p.endswith('text') or p.endswith('label'):
            w = pn.ui.TextInput(**ps)
        elif p.endswith('dash'):
            patterns = list(LineDash)
            w = pn.ui.Select(options=patterns, value=v or patterns[0], **kwargs)
        else:
            continue
        w.jslink(model, value=p)
        widgets.append(w)
    return pn.ui.Column(*sorted(widgets, key=lambda w: w.name))
...
editor=pn.ui.Row(meta_widgets(p), p)
