...
```python
int_slider = pn.widgets.EditableIntSlider(label='Integer Slider', start=0, end=8, step=2, value=4)
...
str_format = pn.widgets.EditableIntSlider(label='Rank', format='0o', start=0, end=100)

tick_format = pn.widgets.EditableIntSlider(label='Count', format=PrintfTickFormatter(format='%d ducks'), start=0, end=100)
