...
Embedding Panel in a FastAPI server has many drawbacks and shortcomings. We now strongly recommend running Panel directly on a FastAPI server as described in the `FastAPI how-to guide`.
:::
...
3. Create a new html template (ex. app2.md) with the same contents as base.html in `examples/apps/fastApi/templates`
