...
```{note}
Note that the global ThreadPool is used to dispatch events triggered by changes in parameters, events (such as click events), `defer_load` callbacks and optionally [`onload` callbacks](../callbacks/load).
```
