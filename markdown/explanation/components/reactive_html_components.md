...
class CustomComponent(ReactiveHTML):
    ...
    _template = """
value: {{value}} ({{ param.value.default }}, {{ param.value.doc }})
<h2>List of parameters

{% for object in param.objects().values() %}{% if loop.index0 < 3 %}
{{ loop.index0 }}. {{object.name}}: {{object.owner[object.name]}} ({{object.default}}, {{object.doc | replace("`", "'")}})<hr/>
{% endif %}{% endfor %}
...
class CustomComponent(ReactiveHTML):
    ...
    _template = """
Jinja literal value. {{ text }}
Javascript template variable. ${text}
"""
...
class CustomComponent(ReactiveHTML):
    v_model = param.String(default="I'm a **model** value. emphasize")
    v_literal = param.String(default="I'm a **literal** value. emphasize")
    v_template = param.String(default="I'm a **template** value. emphasize")
