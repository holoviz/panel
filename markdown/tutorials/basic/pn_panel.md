...
Please notice that `pn.panel` chose a `Vega` pane to display the [Altair](https://altair-viz.github.io/) figure.
...
Please notice that `pn.panel` chose a `HoloViews` pane to display the [hvPlot](https://hvplot.holoviz.org/user_guide/Customization.html) figure.
...
If we place objects in a [*layout*](../../reference/index.rst#layouts) like `pn.Column` (more about layouts later), then the layout will apply `pn.panel` for us automatically.
